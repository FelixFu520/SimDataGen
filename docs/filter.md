...
2. 大面积近白 **且** 低饱和 —— 基本全白
3. 大面积近黑 **且** 低对比 **且** 低饱和 —— 基本全黑、无内容（加低饱和约束，避免误杀"暗调但有暖光 / 丰富色彩内容"的图）
4. 近黑 + 近白占比极高 **且** 低饱和 —— 黑白两极化、几乎无中间调与色彩（如黑底背景 + 过曝纯白植被 / 物体，细节丢失）
...
调参方向：想过滤更严，可调高 `--std-thresh` / `--sat-thresh` 或调低 `--bad-ratio`；想更宽松则反之。
