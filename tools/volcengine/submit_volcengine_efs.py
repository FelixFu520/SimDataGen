...
def create_ml_job(
    ...
    image_url='mlp-cn-beijing.cr.volces.com/preset-images/python:3.12-ubuntu22.04', # 任务使用的镜像
    ...
    efs_mount_path=None,                 # EFS挂载到任务的路径
    efs_prefix=None,                     # 将EFS的哪个路径(SubPath)挂载至任务中
    efs_type="Premium",                  # EFS类型(EfsType),如Premium
    efs_addr="",                         # EFS挂载点域名/IP(Addr),不可为空,否则服务端找不到挂载点
    efs_read_only=False,                 # EFS是否只读,对应图中ReadOnly
    efs_access_point_id=None,            # EFS接入点ID(AccessPointId),如ap-xxxx;走接入点挂载时必填(需fsx白名单)
    ...
    config.ak = ak    # 替换为你的默认AK
    config.sk = sk    # 替换为你的默认SK
    ...
    # ================== 存储配置(挂载TOS、vepfs、EFS):与 is_flexible 无关 ==================
    # 仅根据各存储参数是否提供来动态拼接 storages 列表,任意组合或全部不挂载均可。
    storages = []
    # 仅当提供了TOS参数(桶名)时才挂载对象存储。
    if tos_name:
        storages.append(
    ...
    # 仅当提供了vePFS参数(ID或名称)时才挂载vePFS。
    if vepfs_volume_id or vepfs_name:
        storages.append(
            mlpsdk.StorageForCreateJobInput(
    ...
    # 仅当提供了EFS参数时才挂载EFS。
    # 官方标准结构(Type='Efs')如下,Config.Efs 内字段为 PascalCase:
    #   {"Config": {"Efs": {"FileSystemName","Id","SubPath","EfsType","Addr"}},
    #    "MountPath": ..., "ReadOnly": false, "Type": "Efs"}
    # 当前SDK(volcenginesdkmlplatform20240701)无原生Efs字段,故动态注册 efs->'Efs'。
    # 关键:Addr 必须填 EFS 挂载点的域名或IP(控制台挂载点页可查),不可为空,
    #       否则服务端报 "no available mount point found for VolcEfsId xxx"。
    if efs_id or efs_name:
        efs_config = mlpsdk.ConfigForCreateJobInput()
        # 在实例级别扩展swagger_types/attribute_map,避免污染类(不影响其它实例)
        efs_config.swagger_types = dict(efs_config.swagger_types, efs='object')
        efs_config.attribute_map = dict(efs_config.attribute_map, efs='Efs')
        efs_config.efs = {
            "FileSystemName": efs_name,  # EFS的FileSystemName
            "Id": efs_id,                # EFS的Id
            "SubPath": efs_prefix,       # EFS的SubPath
            "EfsType": efs_type,         # EFS的EfsType,如Premium
            "Addr": efs_addr,            # EFS的Addr,可留空
        }
        storages.append(
            mlpsdk.StorageForCreateJobInput(
                type='Efs',                  # 指定为挂载EFS
                mount_path=efs_mount_path,   # 指定将EFS挂载到任务的路径
                read_only=efs_read_only,     # 是否只读,对应图中ReadOnly
                config=efs_config
            )
        )
    ...
    create_job_req = mlpsdk.CreateJobRequest(
        ...
        # 存储配置(挂载TOS、vepfs、EFS):无任何存储时不传 storage_config
        storage_config=storage_config if storages else None
    )
...
if __name__ == "__main__":
    ...
    # 任务名称
    parser.add_argument("--task_name", type=str, default="test-task", help="任务名称")
    # 执行命令
    parser.add_argument("--command", type=str, default="sleep infinity", help="执行命令")
    # 资源配置
    parser.add_argument("--resource_queue_id", type=str, default="q-20260507105650-5lk49", help="资源队列ID")
    ...
    parser.add_argument("--is_flexible", type=_str2bool, default=True, help="True:灵活配比模式,False:预约模式")
    # TOS存储
    parser.add_argument("--tos_name", type=str, default="drobotics-ailab", help="对象存储名称")
    parser.add_argument("--tos_mount_path", type=str, default="/root/tos", help="对象存储挂载到任务的路径")
    ...
    parser.add_argument("--vepfs_mount_path", type=str, default="/root/vepfs", help="vePFS挂载到任务的路径")
    parser.add_argument("--vepfs_prefix", type=str, default="/fa.fu", help="vePFS前缀")
    # EFS存储
    parser.add_argument("--efs_name", type=str, default="DRoboticsAILab", help="EFS文件系统名称(FileSystemName)")
    parser.add_argument("--efs_id", type=str, default="efs-cnbjc8a589e0f6d6f", help="EFS的Id")
    parser.add_argument("--efs_mount_path", type=str, default="/root/efs", help="EFS挂载到任务的路径")
    parser.add_argument("--efs_prefix", type=str, default="/users/", help="EFS的SubPath")
    parser.add_argument("--efs_type", type=str, default="Premium", help="EFS类型(EfsType),如Premium")
    parser.add_argument("--efs_addr", type=str, default="cnbjc8a589e0f6d6f.3psq7ep69ltkw6csxyuomq6f5.cn-beijing.efs.ivolces.com", help="EFS挂载点域名/IP(Addr),不可为空,否则服务端找不到挂载点")
    parser.add_argument("--efs_read_only", type=_str2bool, default=False, help="EFS是否只读(ReadOnly),true/false")
    parser.add_argument("--efs_access_point_id", type=str, default="ap-ab8d0375", help="EFS接入点ID(AccessPointId),如ap-xxxx;Fsx接入点挂载时必填(需fsx白名单)")
    # 镜像
    parser.add_argument("--private_image_username", type=str, default="D-Robotics@2108796092", help="私有镜像用户名")
    parser.add_argument("--private_image_password", type=str, default=None, help="私有镜像密码")
    ...
    # AK/SK
    parser.add_argument("--ak", type=str, default=None, help="访问密钥AK")
    parser.add_argument("--sk", type=str, default=None, help="密钥SK")
    ...
    if args.private_image_password is None:
        args.private_image_password = os.getenv("VOLC_PASSWD")
    if args.ak is None:
        args.ak = os.getenv("VOLC_AK")
    if args.sk is None:
        args.sk = os.getenv("VOLC_SK")
    assert args.private_image_password is not None, "私有镜像密码不能为空"
    assert args.ak is not None, "访问密钥AK不能为空"
    assert args.sk is not None, "密钥SK不能为空"
    ...
    create_ml_job(
        ...
        efs_id=args.efs_id,
        efs_mount_path=args.efs_mount_path,
        efs_prefix=args.efs_prefix,
        efs_type=args.efs_type,
        efs_addr=args.efs_addr,
        efs_read_only=args.efs_read_only,
        efs_access_point_id=args.efs_access_point_id,
